...
def perform_motion_correction(nifti, bval, bvec, affine):
    """
...
def find_method_file(dataset_dir):
    """
    Dynamically locate the 'method.npy' file in the dataset directory, prioritizing scan 8.

    Parameters:
    - dataset_dir: str, the directory to search in.

    Returns:
    - str: The full path to the method.npy file if found, else None.
    """
    print(f"Searching for method.npy in: {dataset_dir}")
    
    # Specifically check scan 8 first
    prioritized_path = os.path.join(dataset_dir, "8", "method.npy")
    if os.path.exists(prioritized_path):
        print(f"Found method.npy in prioritized scan 8: {prioritized_path}")
        return prioritized_path

    # Fallback to search recursively in case method.npy is elsewhere
    method_files = glob.glob(os.path.join(dataset_dir, "*", "method.npy"), recursive=True)
    print(f"Found method files: {method_files}")
    
    if len(method_files) == 1:
        return method_files[0]
    elif len(method_files) > 1:
        print(f"Warning: Multiple method.npy files found in {dataset_dir}. Using the first one.")
        return method_files[0]
    else:
        print(f"Error: No method.npy file found in {dataset_dir}.")
        return None
...
def process_dataset(diffusion_path, T2_path, output_dir):
    ...
    try:
        ...
        method_file_path = find_method_file(dataset_dir)

        method_file_path = find_method_file(dataset_dir)

# Check if the method file exists
        if not method_file_path:
            print(f"Method file not found in {dataset_dir}. Skipping dataset.")
            return
        ...
        if bval is not None and bvec is not None:
            ...
            dwi_motion_corrected, reg_affines = perform_motion_correction(
                diffusion_nifti, bval, bvec, diffusion_affine
            )
...
def find_diffusion_path(dataset_dir):
    """
    Dynamically find the correct diffusion path within a dataset directory.

    Parameters:
    - dataset_dir: str, path to the '_loaded' directory for the dataset.

    Returns:
    - str: Path to the 'niiobj_1.nii' file, or None if not found.
    """
    # Search for 'niiobj_1.nii' in all subdirectories of the dataset
    diffusion_files = glob.glob(os.path.join(dataset_dir, "*", "pdata", "*", "niiobj_1.nii"))
    
    # If a single file is found, return it
    if len(diffusion_files) == 1:
        return diffusion_files[0]
    elif len(diffusion_files) > 1:
        print(f"Warning: Multiple diffusion files found in {dataset_dir}. Using the first one.")
        return diffusion_files[0]
    else:
        print(f"Error: No diffusion file found in {dataset_dir}.")
        return None


def batch_process(root_dir, output_base_dir):
    """
    Batch process all datasets in the root directory using a single pipeline.
    """
    # Find all '_loaded' folders in the root directory
    dataset_dirs = glob.glob(os.path.join(root_dir, "*_loaded"))

    for dataset_dir in dataset_dirs:
        # Dynamically find the diffusion path for the dataset
        diffusion_path = find_diffusion_path(dataset_dir)

        # Define the T2 path (assuming a standard location within the dataset directory)
        ...
        if diffusion_path and os.path.exists(T2_path):
            ...
            # Process dataset with the pipeline
            print(f"Processing dataset: {dataset_dir}")
            process_dataset(diffusion_path, T2_path, dataset_output_dir)
        else:
            # Log missing files
            if not diffusion_path:
                print(f"Missing diffusion file in dataset: {dataset_dir}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Batch process MRI datasets using a single pipeline.")
    parser.add_argument("--root_dir", required=True, help="Root directory containing '_loaded' datasets")
