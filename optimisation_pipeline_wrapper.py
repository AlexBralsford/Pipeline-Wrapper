...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 29 10:50:07 2024
...
def process_dataset(diffusion_path, T2_path, output_dir):
    """
    Process a single DTI and T2 dataset using multiple pipelines to generate FA and MD maps.
    Compares results across pipelines.
    """
    logger = configure_logging(output_dir)
    pipelines = [
        {"denoising_method": "patch2self", "use_affine": False},
        {"denoising_method": "patch2self", "use_affine": True},
        {"denoising_method": "mppca", "use_affine": False},
        {"denoising_method": "mppca", "use_affine": True},
    ]
    ...
    try:
        ...
        try:
            ...
            print("Diffusion and T2 data loaded successfully.")
        except Exception as e:
        ...
        if not os.path.exists(method_file_path):
            logger.error(f"Method file not found in {method_file_path}. Skipping dataset.")
            return
        ...
        try:
            ...
            logger.error(f"Error loading method file {method_file_path}: {e}")
            return
        ...
        for pipeline in pipelines:
            pipeline_name = f"{pipeline['denoising_method']}_affine_{pipeline['use_affine']}"
            pipeline_output_dir = os.path.join(output_dir, pipeline_name)
            os.makedirs(pipeline_output_dir, exist_ok=True)

            logger.info(f"Running pipeline: {pipeline_name}")

            # File paths for this pipeline
            denoised_save_path = os.path.join(pipeline_output_dir, "image_denoised_temp.nii.gz")
            gibbs_save_path = os.path.join(pipeline_output_dir, "image_gibbs.nii.gz")
            fa_path = os.path.join(pipeline_output_dir, "fa.nii.gz")
            md_path = os.path.join(pipeline_output_dir, "md.nii.gz")
            evals_path = os.path.join(pipeline_output_dir, "evals.nii.gz")
            evecs_path = os.path.join(pipeline_output_dir, "evecs.nii.gz")
            fa_warp_path = os.path.join(pipeline_output_dir, "warp_fa.nii.gz")
            md_warp_path = os.path.join(pipeline_output_dir, "warp_md.nii.gz")

            try:
                # Perform motion correction
                dwi_motion_corrected, reg_affines = perform_motion_correction(
                    diffusion_nifti, bval, bvec, diffusion_affine, use_affine=pipeline["use_affine"]
                )

                if dwi_motion_corrected is None:
                    logger.error(f"Motion correction failed for pipeline: {pipeline_name}")
                    continue

                dwi_motion_corrected_data = dwi_motion_corrected.get_fdata()

                # Perform denoising
                if pipeline["denoising_method"] == "patch2self":
                    denoised_data = perform_patch2self_denoising(
                        dwi_motion_corrected_data, bvals, diffusion_nifti, denoised_save_path
                    )
                elif pipeline["denoising_method"] == "mppca":
                    denoised_data = perform_mppca_denoising(
                        dwi_motion_corrected_data, diffusion_nifti, denoised_save_path
                    )
                else:
                    raise ValueError(f"Unsupported denoising method: {pipeline['denoising_method']}")

                if denoised_data is None:
                    logger.error(f"Denoising failed for pipeline: {pipeline_name}")
                    continue
                ...
                if gibbs_corrected_data is None:
                    logger.error(f"Gibbs correction failed for pipeline: {pipeline_name}")
                    continue
                ...
                logger.info(f"Pipeline: {pipeline_name} | (PIESNO) The mean of the sigma array is: sigma = {mean_sigma}")
                logger.info(f"Pipeline: {pipeline_name} | (PIESNO) The std of the background is: {std_background}")
                logger.info(f"Pipeline: {pipeline_name} | (PIESNO) The noise standard deviation is sigma = {sigma}")
                ...
                logger.info(f"Pipeline: {pipeline_name} | Mean signal={mean_sig}, SNR={snr}")
                ...
                perform_tensor_model_fit(
                    gibbs_corrected_data, diffusion_affine, diffusion_nifti, gtab, fa_path, md_path, evals_path, evecs_path, pipeline_output_dir, logger
                )
                ...
                if upsampled_diffusion_data is not None and warped_moving is not None:
                    logger.info(f"Pipeline: {pipeline_name} | Transformation and warping to T2 completed successfully.")
                    ...
                    logger.info(f"Pipeline: {pipeline_name} | Cross-correlation (before): {cross_corr_value_before}")
                    logger.info(f"Pipeline: {pipeline_name} | Cross-correlation (after): {cross_corr_value_after}")

            except Exception as e:
                logger.error(f"Error in pipeline {pipeline_name}: {e}")

    except Exception as e:
        logger.error(f"Error processing dataset {diffusion_path}: {e}")


def batch_process(root_dir, output_base_dir):
    """
    Batch process all datasets in the root directory using multiple pipelines.
    """
    # Find all _loaded folders in the root directory
    dataset_dirs = glob.glob(os.path.join(root_dir, "*_loaded"))
    for dataset_dir in dataset_dirs:
        # Define paths for diffusion and T2 data
        ...
        print(f"Diffusion path: {diffusion_path}")
        print(f"T2 path: {T2_path}")

        # Check if required files exist
        if os.path.exists(diffusion_path) and os.path.exists(T2_path):
            # Create an output directory for this dataset
            dataset_output_dir = os.path.join(output_base_dir, os.path.basename(dataset_dir))
            os.makedirs(dataset_output_dir, exist_ok=True)

            # Process dataset with all pipelines
            print(f"Processing dataset: {dataset_dir}")
            process_dataset(diffusion_path, T2_path, dataset_output_dir)
        else:
            # Log missing files
            if not os.path.exists(diffusion_path):
                print(f"Missing diffusion file: {diffusion_path}")
            if not os.path.exists(T2_path):
                print(f"Missing T2 file: {T2_path}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Batch process MRI datasets using multiple pipelines.")
